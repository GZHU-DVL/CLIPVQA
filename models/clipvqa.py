...
import numpy as np
from .sat import SpatiotemporalAggregationTransformer
from .vat import VideoAggregationTransformer
from .vat import CandLA
from .fpt import FramePerceptionTransformer
import sys
import warnings
sys.path.append("../")
...
class clipvqa(CLIP):
    def __init__(self,
        ...
        self.prompts_generator = VideoAggregationTransformer(layers=prompts_layers, embed_dim=embed_dim, alpha=prompts_alpha,)
        self.use_cache=use_cache
        self.mit = SpatiotemporalAggregationTransformer(T=T, embed_dim=embed_dim, layers=mit_layers,)

        dpr = [x.item() for x in torch.linspace(0, droppath, vision_layers)] if droppath > 0. else None

        vision_heads = vision_width // 64
        self.visual = FramePerceptionTransformer(
        ...
        # self.token_fc = nn.Linear(T, 1)
        # self.conv2 = nn.Conv2d(in_channels=T, out_channels=512, kernel_size=[512,1], stride=1, bias=False)
        self.conv2 = nn.Conv2d(in_channels=embed_dim, out_channels=embed_dim, kernel_size=[T, 1], stride=1, bias=False)
        # self.head = Mlp(512,act_layer=nn.GELU,out_features=6)
    ...
    def encode_video(self, image):
        ...
        cls_features = cls_features.view(b, t*cls_features.shape[1], -1)
    ...
    def forward(self, image, text):
        ...
        # img_features = self.token_fc(img_features.permute(0, 2, 3, 1)).view(img_features.shape[0],img_features.shape[2],img_features.shape[3])
        # img_features = self.conv2(img_features.permute(0,1,3,2)).view(img_features.shape[0],img_features.shape[2], img_features.shape[3])
        img_features = self.conv2(img_features.permute(0, 3, 1, 2))
        img_features = img_features.reshape(img_features.shape[0],img_features.shape[3],img_features.shape[1])
        if self.use_cache:
        ...
        # text_features = text_features.unsqueeze(1).expand(-1, T,-1, -1)
        # image_text = []
        # for i in range(T):
