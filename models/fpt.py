...
class ClipAggressiveTransformeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # msg_token = self.message_fc_1(msg_token)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, droppath=None, use_checkpoint=False, T=8):
        ...
        self.resblocks = nn.Sequential(*[ClipAggressiveTransformeBlock(width, heads, attn_mask, droppath[i], T) for i in range(layers)])
...
class FramePerceptionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.permute(1, 0, 2)
        cls_x = self.ln_post(torch.cat([x[:, 0, :].view(x.shape[0],1,x.shape[2]),x[:, l:, :]],dim=1))
