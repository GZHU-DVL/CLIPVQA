    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                if self.data_prefix is not None:
                    filename = osp.join(self.data_prefix, filename+'.mp4')
...
def build_dataloader(logger, config):
    ...
    train_pipeline = [
        ...
        dict(type='SampleFrames', clip_len=1, frame_interval=2, num_clips=config.DATA.NUM_FRAMES),
    ...
    train_loader = DataLoader(
        train_data, sampler=sampler_train,  ###   sampler_train
    ...
    val_pipeline = [
        dict(type='DecordInit'),
        dict(type='SampleFrames', clip_len=1, frame_interval=2, num_clips=config.DATA.NUM_FRAMES, test_mode=True),
        dict(type='DecordDecode'),
        dict(type='Resize', scale=(-1, scale_resize)),
